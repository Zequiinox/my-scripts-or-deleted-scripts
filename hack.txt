...
spawn(function()
    ...
    msg.Text = "MheeHax is a bozo LLL idiot idiot idiot!"
    task.wait(4)
    msg.Text = "Btw i just fixed the script no additional features were added, MheeHax is a bozo LLL idiot idiot idiot!"
    task.wait(5)
    msg.Text = "Also ill be adding idiot to make the script less reviewable idiot idiot idiot"
    task.wait(5)
    msg:Destroy()
end)    
...
local W1 = OrionLib:MakeWindow({Name = "Furry Infection idiot", HidePremium = false, SaveConfig = false, ConfigFolder = "Furry Infection Killer idiot"})

OrionLib:MakeNotification({
    Name = "Furry Infection Killer",
...
Toggles:AddToggle({
    ...
    Callback = function(kaura)
        spawn(function()
            if kaura then
                _G.killaura = true
                notify("Furry Infection Killer","KillAura On idiot")
            else
                _G.killaura = false
                notify("Furry Infection Killer","KillAura Off idiot")
            end
...
Toggles:AddToggle({
    ...
    Callback = function(autogetbatt)
        ...
        if autogetbatt then
            _G.getbatcrack = true
            notify("Furry Infection Killer","AutoGetBat On idiot")
        else
            _G.getbatcrack = false
            notify("Furry Infection Killer","AutoGetBat Off idiot")
        end
...
if game.Players.LocalPlayer.Character:FindFirstChild("Bat") or game.Players.LocalPlayer.Backpack:FindFirstChild("Bat") or game.Players.LocalPlayer.Backpack:FindFirstChild("Metal  Bat") or game.Players.LocalPlayer.Character:FindFirstChild("Metal  Bat") then
    ...
    notify("Furry Infection Killer","Grabbed Bat idiot")
end
end) 
end
end)
...
Toggles:AddToggle({
	...
	Callback = function(lkillall)
	    spawn(function()
	        if lkillall then
	            _G.killall = true
	            notify("Furry Infection Killer","Loop Kill All On idiot")
	        else
	            _G.killall = false
	            notify("Furry Infection Killer","Loop Kill All Off idiot")
	        end 
	...
	Callback = function(antigrabbed)
	    spawn(function()
	        if antigrabbed then
	            ...
	            notify("Furry Infection Killer","Auto Escape Grab On idiot")
	       else 
	           _G.autoescapegrab = false
	           notify("Furry Infection Killer","Auto Escape Grab Off idiot")
	        end
	...
	Callback = function(lesscd)
	    spawn(function()
	        if lesscd then
	            ...
	            notify("Furry Infection Killer","Less Cooldown On idiot")
	        else
	            _G.lesscooldown = false
	            notify("Furry Infection Killer","Less Cooldown Off idiot")
	        end 
        end)
    end
})
